...
{
    internal partial class InnerShipStatus : InnerNetObject, IInnerShipStatus
    {
                ...
                case RpcCalls.RepairSystem:
                {
                    ...
                    switch (systemType)
                    {
                        ...
                        case SystemTypes.Doors:
                            ...
                            if (_game.Options.Map != (MapTypes) 2)
                            {
                                _logger.LogWarning($"{nameof(InnerShipStatus)}: Client sent {nameof(RpcCalls.RepairSystem)} for {nameof(SystemTypes.Doors)} on map {_game.Options.Map}");
                                break;
                            }
                        ...
                        case SystemTypes.TopDecontaminationPolus:
                            ...
                            if (_game.Options.Map == 0)
                            {
                                _logger.LogWarning($"{nameof(InnerShipStatus)}: Client sent {nameof(RpcCalls.RepairSystem)} for {systemType} on map {_game.Options.Map}");
                                break;
                            }
